...
contract DeveloperLendPriceOracleV1 is DeveloperLendPriceOracleInterface, OwnableUpgradeSafe {
    ...
    function getPrice(address token) external override(DeveloperLendPriceOracleInterface) view returns (uint){
        Datum storage datum = data[token];
        return datum.value;
    }

    function setPrice(address token, uint price, bool force) external override(DeveloperLendPriceOracleInterface) onlyOwner {
